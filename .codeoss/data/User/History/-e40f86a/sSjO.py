...
# ==========================================
# 1. CONFIGURATION (SECURE SETUP)
# ==========================================

# Keys are now pulled dynamically from the secure environment
...
def handle_audio(chat_id, file_id):
    ...
    send_interim(chat_id, t(lang, 
        "🎙️ **[Analysis]** Analyzing acoustics for Deepfakes and scanning script for Macau Scams...", 
        "🎙️ **[Analisis]** Menganalisis akustik untuk Deepfake dan menyemak skrip untuk Macau Scam...", 
        "🎙️ **[分析]** 正在分析深度伪造声学并扫描澳门骗局脚本..."))
    ...
    prompt = (
        ...
        "DECISION LOGIC:\n"
        "- Output '🔴' (CRITICAL RISK) if it is an obvious AI Deepfake OR a clear scam script (threats, impersonating police/banks, asking for money).\n"
        "- Output '🟡' (SUSPICIOUS) if the audio is heavily distorted making it hard to verify, or if the conversation is highly unusual/aggressive but not definitively a scam.\n"
        "- Output '🟢' (CLEAN) for normal, safe human conversations.\n\n"
    ...
    if "🔴" in response.text:
        send_sos(chat_id, t(lang, 
            "🚨 SOS: User received a Critical Voice Threat (Deepfake or Scam Script)!", 
            "🚨 SOS: Pengguna menerima Ancaman Suara Kritikal (Deepfake atau Skrip Scam)!", 
            "🚨 SOS: 用户收到严重语音威胁（深度伪造或诈骗脚本）！"))
        # 🛡️ LOG TO EVIDENCE VAULT
    ...
    try:
        ...
        if vt_res.status_code == 200:
            ...
            if malicious_count > 0:
                ...
                # 🛡️ LOG TO EVIDENCE VAULT
                log_threat_to_vault(chat_id, 'URL_VIRUSTOTAL', url_to_check, 'HIGH', f'Flagged by {malicious_count} VT Engines.')
        ...
        if report_data and "error" not in report_data:
            ...
            send_reply(chat_id, f"⚠️ AwasBot-AI Error: {error_text}")
...
def check_apk(chat_id, fid, name):
    ...
    send_interim(chat_id, t(lang, 
        f"🦠 **[Layer 1]** Querying VirusTotal for Malware Signatures...", 
        f"🦠 **[Lapisan 1]** Menyemak VirusTotal untuk Tandatangan Hasad...", 
        f"🦠 **[第一层]** 正在查询 VirusTotal 恶意软件签名..."))
    ...
    send_interim(chat_id, t(lang, 
        f"📦 **[Layer 2]** Unpacking APK & Extracting Permissions...", 
        f"📦 **[Lapisan 2]** Menyahmampat APK & Mengekstrak Kebenaran...", 
        f"📦 **[第二层]** 正在解包 APK 并提取权限..."))
    ...
    send_interim(chat_id, t(lang, 
        f"🧠 **[Layer 3]** Running AI Forensic Analysis on behavior...", 
        f"🧠 **[Lapisan 3]** Menjalankan Analisis Forensik AI pada tingkah laku...", 
        f"🧠 **[第三层]** 正在对应用行为进行 AI 取证分析..."))
    ...
    try:
        ...
        if int(score) > 70 or malicious > 0:
            send_sos(chat_id, t(lang, 
                f"🚨 **SOS ALERT**: High Risk APK (Banking Trojan) detected! {name}", 
                f"🚨 **AMARAN SOS**: APK Berisiko Tinggi dikesan! {name}", 
                f"🚨 **SOS 警报**: 发现高风险 APK (网银木马)！{name}"))
            
            # 🛡️ LOG TO EVIDENCE VAULT
            vault_score = max(int(score), 100) if malicious > 0 else score
            log_threat_to_vault(chat_id, 'APK_MALWARE', name, vault_score, report_data.get('summary', 'Banking Trojan detected.'))
        ...
        caption = t(lang,
            ...
            f"**包名：** `{package_name}`\n"
            f"**风险评分：** {score}/100\n\n"
            f"**AI 摘要：** {report_data.get('summary')}\n\n"
            f"**结论：** {report_data.get('verdict_en')}"
        )
        send_reply(chat_id, caption + get_disclaimer(lang))
        
    except Exception as e:
        send_reply(chat_id, f"⚠️ Analysis Error: {str(e)}")
...
def check_pdf(chat_id, fid):
    ...
    if vt_res.status_code == 200:
        ...
        if malicious > 0:
            ...
            # 🛡️ LOG TO EVIDENCE VAULT
            log_threat_to_vault(chat_id, 'PDF_MALWARE', 'PDF Document', 'CRITICAL', f'Flagged by {malicious} Antivirus Engines')
    ...
    if "🔴" in response.text:
        send_sos(chat_id, t(lang, 
            ...
            "🚨 SOS: 发现诈骗 PDF！"))
        # 🛡️ LOG TO EVIDENCE VAULT
        log_threat_to_vault(chat_id, 'PDF_PHISHING', 'PDF Document', 'CRITICAL', 'Extortion or Phishing content detected.')
...
def check_general_document(chat_id, fid, name):
    ...
    if vt_res.status_code == 200:
        ...
        if malicious > 0:
            ...
            # 🛡️ LOG TO EVIDENCE VAULT
            log_threat_to_vault(chat_id, 'DOC_MALWARE', name, 'CRITICAL', f'Flagged by {malicious} Antivirus Engines')
...
def log_threat_to_vault(reporter_id, threat_type, target, risk_level, reason):
    ...
    try:
        ...
        print(f"🔒 EVIDENCE VAULT: Logged {threat_type} threat.")
    except Exception as e:
...
def handle_bank_webhook(data):
    ...
    if score <= 0.3:
        ...
        if victim_chat_id:
            ...
            # 🛡️ LOG B2B FRAUD TO EVIDENCE VAULT
